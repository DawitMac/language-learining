...
const SidebarComponent = ({ open , setOpen } : { open : boolean , setOpen : React.Dispatch<React.SetStateAction<boolean>>}) => {
  ...
  return ( 
    <motion.div animate={{ x: 0 , opacity : 1 , zIndex : 10 }} initial={{ x : -200 , opacity : 0}} transition={{ duration : 0.5 , ease : "easeInOut"}} className='relative flex flex-col items-center justify-start w-[200px] h-screen gap-12 border-r-2 border-[#F4F4F5] z-10 bg-white'>
    ...
    <ul className='flex flex-col items-start justify-start gap-6 list-none'>
        {
            sidebar.map((list , i)=>(
                <li onClick={()=>{ 
                   setOpen(false)
               }} key={i} className='text-sm font-normal hover:text-[#5D87FF] trnasition-all duration-300 ease-in-out cursor-pointer'>
    ...
    <div className='absolute bottom-20 left-8 flex flex-col items-start justify-start gap-6'>
        ...
        <p className='text-sm font-normal'>Setting</p>
        </span>
       <form action={logOut}>
