...
const Hero = () => {
  return (
    <>
     <motion.div initial={{ opacity : 0 }} animate={{ opacity : 1}} exit={{ opacity : 0}} className='mt-32 md:mt-24  lg:mt-32 flex flex-col md:flex-row items-center md:justify-around justify-center w-screen md:my-8 my-16  md:gap-0 gap-4'>
        <motion.div initial={{ x : -100 , opacity : 0}} animate={ {x: 0 , opacity : 1, transition: { type : "slide" , duration : 0.2 , delay : 0.002} }} className='flex flex-col  items-start justify-center text-start md:w-[40%] w-[80%] '>
            ...
            <span className='flex gap-8'>
              <Link href='/signup'>
               <button className='text-sm md:py-2 py-3 lg:px-12 md:px-6 px-6 bg-[#5D87FF] text-white hover:scale-105 transition-all duration-300 ease-out rounded-lg'>Get Started</button>
              </Link>
